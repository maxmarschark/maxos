...
export function transformOrder(order, userId) {
  return {
    ...
    subtotal_amount: Number(order.subtotalAmount ?? order.subtotal_amount) || 0,
    discount_amount: Number(order.discountAmount ?? order.discount_amount) || 0,
    order_amount: Number(order.orderAmount ?? order.order_amount) || 0,
    commission_percent: Number(order.commissionPercent ?? order.commission_percent) || 0,
    commission_amount: Number(order.commissionAmount ?? order.commission_amount) || 0,
